...
class ProcessControllerTest {
    ...
    void testDeployProcess_Success() throws Exception {
        ...
        doNothing().when(processDefinitionService).deployProcess(anyString(), anyString());
        ...
        verify(processDefinitionService, times(1)).deployProcess(eq("测试流程"), eq("<xml>content</xml>"));
    }
    ...
    void testDeployProcess_Exception() throws Exception {
        ...
        doThrow(new RuntimeException("XML格式错误"))
                .when(processDefinitionService).deployProcess(anyString(), anyString());
    ...
    void testGetProcessDefinitions_Success() throws Exception {
        ...
        when(processDefinitionService.getProcessDefinitions()).thenReturn(definitions);
        ...
        verify(processDefinitionService, times(1)).getProcessDefinitions();
    }

    @Test
    void testDeleteProcessDefinition_Success() throws Exception {
        doNothing().when(processDefinitionService).deleteProcessDefinition(anyString());

        mockMvc.perform(delete("/process/definition/deployment1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200));

        verify(processDefinitionService, times(1)).deleteProcessDefinition("deployment1");
    }
    ...
    void testStartProcess_Success() throws Exception {
        ...
        when(processDefinitionService.startProcess(anyString(), anyString())).thenReturn(instance);
        ...
        verify(processDefinitionService, times(1)).startProcess(eq("testProcess"), eq("business1"));
    }
    ...
    void testGetRunningInstances_Success() throws Exception {
        ...
        when(processDefinitionService.getRunningInstances()).thenReturn(instances);
        ...
        verify(processDefinitionService, times(1)).getRunningInstances();
    }

    @Test
    void testGetCompletedInstances_Success() throws Exception {
        List<ProcessInstanceVO> instances = new ArrayList<>();
        when(processDefinitionService.getCompletedInstances()).thenReturn(instances);
        ...
        verify(processDefinitionService, times(1)).getCompletedInstances();
    }

    @Test
    void testGetProcessDiagram_Success() throws Exception {
        byte[] diagramBytes = new byte[]{1, 2, 3, 4, 5};
        when(processDefinitionService.generateDiagram(anyString())).thenReturn(diagramBytes);
        ...
        verify(processDefinitionService, times(1)).generateDiagram("process1");
    }

    @Test
    void testGetProcessDiagram_Exception() throws Exception {
        when(processDefinitionService.generateDiagram(anyString()))
                .thenThrow(new RuntimeException("流程实例不存在"));

        mockMvc.perform(get("/process/diagram/invalid-process"))
                .andExpect(status().isInternalServerError());

        verify(processDefinitionService, times(1)).generateDiagram("invalid-process");
    }

    @Test
    void testSaveElementExtension_Success() throws Exception {
        doNothing().when(processDefinitionService).saveElementExtension(
        ...
        verify(processDefinitionService, times(1)).saveElementExtension(
    ...
    void testGetElementExtension_Success() throws Exception {
        ...
        when(processDefinitionService.getElementExtension(anyString(), anyString())).thenReturn(result);
        ...
        verify(processDefinitionService, times(1)).getElementExtension("def1", "element1");
    }

    @Test
    void testDeleteElementExtension_Success() throws Exception {
        doNothing().when(processDefinitionService).deleteElementExtension(anyString(), anyString());

        mockMvc.perform(delete("/process/extension/def1/element1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200));

        verify(processDefinitionService, times(1)).deleteElementExtension("def1", "element1");
    }

    @Test
    void testGetAllElementExtensions_Success() throws Exception {
        List<BpmnElementExtension> extensions = new ArrayList<>();
        when(processDefinitionService.getAllElementExtensions(anyString())).thenReturn(extensions);
        ...
        verify(processDefinitionService, times(1)).getAllElementExtensions("def1");
    }

    @Test
    void testBatchSaveElementExtensions_Success() throws Exception {
        doNothing().when(processDefinitionService).batchSaveElementExtensions(
                anyString(), anyList());
        ...
        verify(processDefinitionService, times(1)).batchSaveElementExtensions(eq("def1"), anyList());
    }
}
